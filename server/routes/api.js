...
const validateQuery = (res, count) =>{
	...
	if (!count || !Number(count)) {
		return sendErrorResponse(res, 400, 'Missing or invalid query param count')
	}
...
router.post("/:userId/posts", validateUserToken, async (req, res, next) => {
	...
	try {
		...
		console.error(e);
		return sendErrorResponse(res, 500, 'Could not create the post at this time.')
	}
	// Send a response
...
//
router.get("/:userId/posts/:id/comments", validateUserToken, async (req, res, next) => {
...
//
router.post("/:userId/posts/:id/comments", validateUserToken, async (req, res, next) => {
